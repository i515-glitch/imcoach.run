...
export function Navbar({ currentStep, setStep, activeGoal, onReset, user, onOpenAuth, onLogout, activeService = 'run', onSelectService }) {
  const isHub = activeService === 'hub';
  ...
  return (
    <header style={{
      ...
      <div 
        onClick={() => {
          if (onSelectService) {
            onSelectService(isHub ? 'run' : 'hub');
          } else {
            setStep(activeGoal ? 'coaching' : 'goal');
          }
        }}
      ...
      >
        <div style={{
          ...
          background: isHub 
            ...
            : 'linear-gradient(135deg, var(--accent-primary) 0%, #38bdf8 100%)',
          display: 'flex',
          alignItems: 'center',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          <span style={{ fontSize: '20px', fontWeight: '900', letterSpacing: '-0.03em', color: '#fff' }}>
            imcoach<span style={{ color: 'var(--accent-primary)' }}>{isHub ? '.co' : '.run'}</span>
          </span>
          {!isHub && onSelectService && (
            <button
              onClick={(e) => { e.stopPropagation(); onSelectService('hub'); }}
              style={{
                padding: '3px 8px',
                borderRadius: '8px',
